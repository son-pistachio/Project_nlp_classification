...
class distilBertLightningModel(LightningModule):
    def __init__(self, bert_pretrained, num_labels=num_classes, lr=lr, total_steps=None):
        super(distilBertLightningModel, self).__init__()
...
wandb_logger = WandbLogger(project="bert-classification", log_model=True, name="distilBERT_"+now_sys)

checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath='checkpoints/distilbert/',
    filename='distilbert-{epoch:02d}-{val_loss:.2f}',
    save_top_k=1,
    mode='min',
)
...
bert_model = distilBertLightningModel(CHECKPOINT_NAME, total_steps=total_steps)

trainer.fit(bert_model, train_loader, val_loader)
trainer.test(bert_model, test_loader)
trainer.save_checkpoint("checkpoints/distilbert/final_model.ckpt")
